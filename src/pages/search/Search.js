    ...
    return(
        <>
            <ThemeProvider theme={darkTheme} >
                ...
                <Tabs onChange={(event,newValue)=>{setType(newValue);setPage(1);}} value={type} indicatorColor='primary' textColor='primary' style={{paddingBottom:5}} >
            ...
            <div className='trending' >
                {
                    content?content.map(c=><SingleContent key={c.id} id={c.id} poster={c.poster_path} title={c.title || c.name} date={c.first_air_date || c.release_date} media_type={type?'tv':'movie'} vote_average={c.vote_average} />):console.log('nope')
                }
                {
